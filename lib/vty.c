...
static int vty_mgmt_get_result_notified(
	struct mgmt_fe_client *client, uintptr_t usr_data, uint64_t client_id,
	uintptr_t session_id, uintptr_t session_ctx, uint64_t req_id,
	...
	char *data, char *errmsg_if_any)
{
	...
	if (!success) {
		zlog_err("GET request for client 0x%" PRIx64
			 " failed, Error: '%s'",
			 client_id, errmsg_if_any ? errmsg_if_any : "Unknown");
		vty_out(vty, "ERROR: GET request failed, Error: %s\n",
			errmsg_if_any ? errmsg_if_any : "Unknown");
		vty_mgmt_resume_response(vty, CMD_WARNING);
		return -1;
	}

	MGMTD_FE_CLIENT_DBG("GET request succeeded, client 0x%" PRIx64
			    " req-id %" PRIu64,
			    client_id, req_id);
...
static struct mgmt_fe_client_cbs mgmt_cbs = {
	...
	.get_notify = vty_mgmt_get_result_notified,
};
...
int vty_mgmt_send_get_req(struct vty *vty, bool has_config, bool config,
			  ...
			  const char *xpath)
{
	...
	if (mgmt_fe_send_get_req(mgmt_fe_client, vty->mgmt_session_id,
				 vty->mgmt_req_id, has_config, config,
				 datastore, lyd_to_mgmt_format(format),
				 xpath)) {
		zlog_err("Failed to send GET to MGMTD for req-id %" PRIu64 ".",
			 vty->mgmt_req_id);
		vty_out(vty, "Failed to send GET to MGMTD!\n");
		return -1;
	}

	vty->mgmt_req_pending_cmd = "MESSAGE_GET_REQ";
