				     ...
				     const char *error_if_any)
{
	...
	if (success) {
		LY_ERR err = lyd_print_mem(&data, result, format,
					      ...
					      Mgmtd__FeGetReq *get_req)
{
	...
	/* Include config if "has_config" is not set or set to "true" */
	config = !get_req->has_config || get_req->config;
	if (config && ds_id == MGMTD_DS_OPERATIONAL) {
		/*
		 * When config is requested from the operational datastore,
		 * use running datatore instead.
		 */
		ds_ctx = mgmt_ds_get_ctx_by_id(mm, MGMTD_DS_RUNNING);

	}
	/* Include state if "has_config" is not set or set to "false" */
	state = !get_req->has_config || !get_req->config;
	if (state)
		clients = mgmt_be_interested_clients(get_req->xpath, false);

	/*
	 * Create a GET request under the transaction.
	 */
	if (mgmt_txn_send_get_req(session->txn_id, req_id, ds_id, ds_ctx, config,
				  state, clients, format, get_req->xpath)) {
		fe_adapter_send_get_reply(session, ds_id, req_id, false, format,
					  NULL,
					  "Request processing for GET failed!");
